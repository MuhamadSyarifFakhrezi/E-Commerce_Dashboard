...
def extract_zip_to_csv(zip_file_path, extract_folder):
    with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
        # Ekstrak semua file ke dalam folder tertentu
...
# Ekstrak file ZIP
extract_zip_to_csv(zip_file_path, extract_folder)

# Dapatkan semua file CSV dalam folder ekstraksi
csv_files = [f for f in os.listdir(extract_folder) if f.endswith('.csv')]

# Simpan setiap DataFrame ke dalam dictionary dengan nama file sebagai kunci
dataframes = {}
...
# Sekarang, variabel 'dataframes' berisi setiap DataFrame dengan nama file sebagai kunci
# Anda dapat mengakses DataFrame menggunakan dataframes['nama_file.csv']


# In[14]:


dataframes['e-commerce_data.csv'].head()


# In[15]:


dataframes['map_df.csv'].head()


# In[16]:


df = dataframes['e-commerce_data.csv']
